...
(function () {
  ...
  // "Hồ Sơ Doanh Nghiệp" dropdown — same interaction as the home page
  // (js/main.js initUI): click to toggle, click outside / Esc to close.
  function initNavDropdown() {
    const items = document.querySelectorAll('.nav-item');
    items.forEach((item) => {
      const toggle = item.querySelector('.nav-item__toggle');
      if (!toggle) return;
      const close = () => { item.classList.remove('is-open'); toggle.setAttribute('aria-expanded', 'false'); };
      ...
      item.addEventListener('keydown', (e) => { if (e.key === 'Escape') { close(); toggle.focus(); } });
    });
    document.addEventListener('click', (e) => {
      if (e.target.closest('.nav-item')) return;
      items.forEach((item) => {
        item.classList.remove('is-open');
        const t = item.querySelector('.nav-item__toggle');
